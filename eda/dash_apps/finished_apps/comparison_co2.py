...
app = DjangoDash('Comparison_CO2', external_stylesheets= [dbc.themes.BOOTSTRAP])
...
def clean_data(mission_id):
    """Returns CO" (ISS/Ground) and days after launch for a given mission"""
    ...
    mission_dict = {mission_id : np.array([days_after_launch_col.values, df_launch['CO2_ppm_Ground'].values, df_launch['CO2_ppm_ISS'].values])}
    return mission_dict
...
def update_dataframe(checklist):
    ...
    for mission_id in updated_checklist:
        if mission_id not in dict_store_co2.keys():
            mission_dict = clean_data(mission_id)
            dict_store_co2.update(mission_dict)
            print(mission_id)
        else:
            print('Data already loaded!')
    print(dict_store_co2.keys())
    return updated_checklist, dict_store_co2
