...
app = DjangoDash('Comparison_RH', external_stylesheets= [dbc.themes.BOOTSTRAP])
...
def clean_data(mission_id):
    """Returns RH (ISS/Ground) and days after launch for a given mission"""
    ...
    mission_dict = {mission_id : np.array([days_after_launch_col.values, df_launch['RH_percent_Ground'].values, df_launch['RH_percent_ISS'].values])}
    return mission_dict
...
def fig_plot_line(dict, checklist, location):
    ...
    title={
        'text': "Relative Humidity {}".format(location),
        'y':0.9,
        'x':0.5,
        'xanchor': 'center',
        'yanchor': 'top'}
